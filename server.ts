...
async function startServer() {
  ...
  // Executor de Scripts Python dinâmico para listar projetos/atividades
  function executePythonScript(type: "themes" | "projects", folder?: string): any {
    try {
      let cmd = `python3 listar_projetos.py --type ${type}`;
      if (folder) {
        cmd += ` --folder "${folder}"`;
      }
      ...
      console.error("[Python Executor Error]:", err.message);
      throw err;
    }
  }
  ...
  app.get("/api/themes", (req, res) => {
    try {
      console.log("[Themes API] Varrendo pastas de projetos via Script Python...");
      const themes = executePythonScript("themes");
      res.json(themes);
    } catch (err: any) {
      console.warn("[Themes API] Falha ao rodar Python, usando fallback local:", err.message);
      try {
        const themes = scanProjectFolders();
        res.json(themes);
      } catch (fallbackErr: any) {
        res.status(500).json({ error: "Erro ao varrer pastas de projetos: " + fallbackErr.message });
      }
    }
  });
  ...
  app.get("/api/projects", (req, res) => {
    ...
    try {
      console.log(`[Projects API] Listando arquivos da pasta '${folder}' via Script Python...`);
      const output = executePythonScript("projects", folder);
      res.json(output);
    } catch (err: any) {
      console.warn(`[Projects API] Falha ao rodar Python para pasta '${folder}', usando fallback local:`, err.message);
